...
public class FoursquareController implements IFoursquareConstants, Response.Listener<JSONObject>, Response.ErrorListener{
  ...
  public void getAttractions(String area, List venuesList, RecyclerView.Adapter venuesRecyclerAdapter, Context context){
    ...
    return new Venue(name, id, iconUri, lat, lng, city, country, address, categories);
  }
  ...
  private String buildIconUri(JSONArray venueCategories) {
    ...
    try {
      iconUri =  venueCategories.getJSONObject(0).getJSONObject(ICON)
              .getString(PREFIX) + VENUE_ICON_SIZE + venueCategories
